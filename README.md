...
- **CRS na tela** — o **sistema de referência** do produto aberto aparece no painel (código **EPSG**, nome do sistema, unidade, tamanho do pixel e extensão) e num selo sobre o mapa, que você liga ou desliga. É o que se confere **antes** de colar os pontos da estatística zonal: pontos e imagem têm de estar no mesmo sistema. Se o app não souber reprojetar aquele CRS, ele avisa ali mesmo.
- **Recorte do ortomosaico** — marque a área de interesse (polígono ou retângulo, ou reaproveite a área do voo) e **recorte** o raster; passe a visualizar, calcular estatística e gerar índice **só na parte útil**, e salve o recorte (PNG + worldfile). **↺ Imagem inteira** restaura o original. Ao baixar — recorte ou imagem inteira — o arquivo sai **com o que estiver marcado no mapa** (buffers da estatística por pontos e a área desenhada); desmarque **«Incluir as marcações»** para levar o raster limpo ao QGIS. A **espessura** das marcações (mapa e arquivo) é escolhida no mesmo painel.
